...
description: Jinterface is a way to make Java programs behave like Erlang nodes to make the communication b/w them compatible
...
`Jinterface` is a way to make `Java` programs behave like `Erlang` nodes to make the communication b/w them compatible. 
...
In this case, you will need to install `Erlang` from the source with `Java` installed on the path. 
