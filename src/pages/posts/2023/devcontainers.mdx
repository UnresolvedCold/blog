...
1. We want to initialize the databases as per the `./misc/database_creator.sql`. 
...
> Note: Here I haven't used `git` and `gh` features because the image `mcr.microsoft.com/devcontainers/java:8-bullseye` already comes with these tools. 
