...
To add title to the image you'll need to create a server to generate the image with the title.
...
After we are done with the modificatons, we'll sent the image as response. 

We will use `Flask` as the server. 
`Pillow` module will help us modify the image. 
And to download the initial image we were using, we will use `get` method of the good old module called `requests`.
...
> You can serve your html content from this server. But for simplicity, let's keep them as separate entity. 
...
Now let's expose this to public temprorarily using `localhost.run`. This time we will expose the port`5000`.
