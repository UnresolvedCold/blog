...
| Topics | Description |
| ------ | ----------- |
| [The Problem](#the-problem) | Merge multiple git repos as one retaining all the histories |
| [A bit about git](#a-bit-about-git) | Introduction on blob, tree and commits |
| [Rewriting history](#rewriting-history) | Rewriting histories using filter-branch. Moving a file inside a directory while retaining git histories |
| [Merging multiple repos into one](#merging-multiple-repos-into-one) | Step by step process on merging 2 git repos into a larger parent repo retaining histories |
| [Merging using Shopsys monorepo-tools](#merging-using-shopsys-monorepo-tools) | Merging 2 git repos into a larger larger parent repo using monorepo tools by shopsys |
...
And for every project we maintianed a separate `git` repository hosted on Bitbucket.
...
We wanted to combine all these repos as a subdirectory of a new larger parent project. 
...
As we are using maven, all you require to do is create a master `POM.XML` that will contain all the sub-repos as submodules. 
...
At GreyOrange, we maintain a `develop` branch and certain `release` branches for each repo.
We wanted to merge their histories so that we can have all our commits on the same branch as they were originally. 
We cannot retain the commit hash as the folder structure has changed, but we want to retain all our commit messages. 

We also have many feature branches but they were mostly independent, so merging wasn't required for them. 

## A bit about git 

Running `git init` creates a new `.git` directory. This directory contains everything `git` needs to do it's magic.
...
[`filter-branch`](https://git-scm.com/docs/git-filter-branch) can be used to rewrite the entire history of a file. 
This will create a new blob, tree and commit for everything once again. 
Rewriting history will not hamper the commit data. 
Hashes will change but the entire inforamtion about the author, committer, etc will remain unchanged. 
...
And add the remotes of our repo we want to merge. 
...
And atlast, we can manipulate the git history into thinking the codes were present in the subdirectory from the beginning.
...
Checkout the branch of the first repository and modify the index file such that repo1 files are inside `repo1` directory. 
...
The above looks good. Atleast we can see the files related to repo1 in `repo1` subdirectory. 
...
We can see the files and commit histories both have been migrated to repo1 succcessfully. 
Now we can do the same process for repo2. 
And in case you have some other branch you'll need to loop the process for other branches too.

But first we'll need to delete the original references to create new ones. 
And we also want to merge the previous reference with the new ones.
...
Now we are ready for merging repo2. Again we'll need to reset the HEAD before merging.
For merging 2 repos, we'll create a new branch called 'main' and will merge both repos main branch one by one. 
...
This is the basic fundamental on how you can merge 2 repos into one retaining all the commit histories.
Now, ofcourse we never want to manually redo this for all the repos & for all the branches.
...
There are many such merger tools available online, we used [monorepo-tools by shopsys](https://github.com/shopsys/monorepo-tools). 
There were several tweaks we did for our use case. One was, by default this tool only merges the `master` branch. 
...
There are variety of other things that you can do with this tool like splitting a repo into multiple repos (reverse of what we did here). 
...
040000 tree d83102ebb26f925b8e087c821e49ca910f1750a6	repo1
040000 tree 7801ea011e82b38c3f3de145571ad75536d5bd5c	repo2
> git cat-file -p d83102ebb26f925b8e087c821e49ca910f1750a6
100644 blob 142ee887e3184237ac17f918498bda405eeb5fc1	README.md
> git cat-file blob 142ee887e3184237ac17f918498bda405eeb5fc1
# test-repo-1

This is a test repository.
> git cat-file -p 7801ea011e82b38c3f3de145571ad75536d5bd5c
100644 blob 18c9019fe2e7d5e8151db4cb5f1d10307c8547ec	README.md
This is a test repository.
...
Merging of multiple repos into a monorepo structure can be accomplished by rewriting `git` histories.
This can be done manually by fetching a repo and tweaking the git index believing it belongs to a subdirectory rather than the main repo itself. 
Another way is by using open source tools like [shopsys monorepo tools](https://github.com/shopsys/monorepo-tools) using which merging can be done in a few steps.
