...
And to avoid any circular dependency issue, we also wanted to combine all these related repositories into a mono-repo system.

The idea is to have a parent git repository which will contain all these repositories as a subdirectory.
...
As we were using maven, all we required was to create a master `POM.XML` that will contain all the sub-repos as submodules. 
...
We wanted to merge their histories such that we can have all our commits on the same branch as they were originally. 
...
These are the 3 basic elements that defines all the `git` functionality. 
...
You can view a blob contents using `git cat-file`. 
...
There are variety of other things that you try with this tool like splitting a repo into multiple repos (reverse of what we did here). 
